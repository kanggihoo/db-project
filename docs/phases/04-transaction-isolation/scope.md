...
Phase 4의 목표는 PostgreSQL에서 `READ COMMITTED`, `REPEATABLE READ`, `SERIALIZABLE` 격리 수준이 읽기 일관성과 동시 갱신 충돌에 어떤 차이를 만드는지 재현하고 기록하는 것이다.
...
| Lost Update | 같은 Product SKU row에 대한 naive read-modify-write | REPEATABLE READ 이상에서 조용한 덮어쓰기가 아니라 concurrent update failure가 발생하는지 |
