...
| server log excerpt | concurrent update failure, serialization failure, transaction rollback 원인 확인 |
...
- PostgreSQL REPEATABLE READ 이상에서 같은 row 동시 갱신이 concurrent update failure 또는 serialization failure로 실패하면 Lost Update가 조용히 발생하지 않는 것으로 기록한다.
