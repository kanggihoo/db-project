...
Phase 12에서는 outbox-publisher가 `outbox_event`를 직접 polling한다. Phase 13에서는 같은 Outbox 테이블을 유지하되, polling publisher 대신 Debezium 같은 CDC 도구가 PostgreSQL WAL 변경을 Kafka로 스트리밍하도록 확장한다. 이 Phase에서는 Postgres WAL, replication slot, Debezium, Kafka topic, consumer replay를 학습한다.
...
```text
orders/payments/outbox_event 변경
  → PostgreSQL WAL
  → Debezium connector
  → Kafka topic
  → consumer
...
| 3 | 이벤트 생성 | 주문 생성 또는 결제 완료로 outbox row insert | Kafka message |
...
Phase 13의 관심사는 결제 장애 대응 전체가 아니다. PG timeout, webhook, 보상 트랜잭션은 별도 결제 프로젝트의 주제이며, 이 Phase에서는 Outbox 이벤트가 CDC를 통해 누락 없이 Kafka로 전달되고 consumer가 재처리 가능한지만 검증한다.
...
| 구분 | Phase 12 Outbox | Phase 13 CDC + Kafka |
| --- | --- | --- |
| 이벤트 저장 | 애플리케이션 트랜잭션에서 `outbox_event` insert | 동일 |
| 발행 방식 | polling publisher | Debezium CDC |
| 핵심 장애 | publisher down, Kafka down, PENDING backlog | connector down, replication slot lag, consumer lag |
| 핵심 지표 | outbox pending count, retry count, oldest pending seconds | replication lag, connector status, consumer lag |
